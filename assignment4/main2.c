...
void _freeBST(struct Node *node) //clearBST and deleteBST send this function the tree->root node.
{
	if(node != 0)
	{
		_freeBST(node->left);
...
struct Node *_addNode(struct Node *cur, TYPE val)
{
	...
	if(compare(cur->val, val) == 1) //val is greater than or equal to cur->val, shift right 
	{
		cur->left = _addNode(cur->left, val);
	}
	else //val is less than cur->val, shift left
	{
...
{
	struct Node * iter = tree->root; //create a pointer to iterate through tree

	while(iter != 0)
	{
		if(compare(iter->val, val) == 0) 
		{
			return 1; //return true, iter->val == val
		}
		if(compare(iter->val, val) > 0) //iter->val is greater than val, so move left 
		{
			iter = iter->left;
		}
...
struct Node *_removeLeftMost(struct Node *cur)
{
	...
	if(cur->right != 0)
	{
		//after left is removed, return right
		//struct Node *tempRight = cur->right;
		struct Node *tempRight = _removeLeftMost(cur->right);
		free(cur);	
		return tempRight;
	}
	free(cur);
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
	...
	// found the node we need to remove
 ...
 ***************************************************************************************************************************/
/*
   function to built a Binary Search Tree (BST) by adding.costs in this specific order
